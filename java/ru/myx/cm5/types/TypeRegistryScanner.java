...
public class TypeRegistryScanner implements Runner, Runnable, TypeRegistry {
	...
	/** FIXME: common types should be compiled once, not for every server... then this compiled code
	 * should be put in separate type instances for each server. */
	...
	void scan() {
		...
		final Map<String, Entry> files;
		{
			...
			/** our local type files (will replace common type files if there is name conflict) */
		...
		/** check for early exit: nothing changed */
		{
			...
			checkFiles.addAll(this.allDates.keySet());
			{
				...
				/** early exit: nothing changed and previous key set is the same as new one */
		...
		/** Now we are going to create and initialize new actual TypeRegistry to reflect new set of
		 * types. */
		...
		/** Update types in the server's root context */
		{
			...
			checkTypes.addAll(this.allRegistered.keySet());
			{
				...
				/** Remove some server's root context properties for deleted types
				 *
				 * if any, of course */
