 ...
 *         Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
final class FormDeleteGroupConfirmation extends AbstractForm<FormDeleteGroupConfirmation> {
	...
	FormDeleteGroupConfirmation(final String path, final String key) {
		this.key = key;
		this.setAttributeIntern( "id", "confirmation" );
		this.setAttributeIntern( "title",
				MultivariantString.getString( "Do you really want to delete this group?",
						Collections.singletonMap( "ru", "Удалить эту группу?" ) ) );
	...
	public Object getCommandResult(final ControlCommand<?> command, final BaseObject arguments) {
		if (command == FormDeleteGroupConfirmation.DELETE) {
			if (Convert.MapEntry.toBoolean( this.getData(), "confirmation", false )) {
				Context.getServer( Exec.currentProcess() ).getAccessManager().deleteGroup( this.key );
				return null;
			}
	...
	public ControlCommandset getCommands() {
		return Control.createOptionsSingleton( FormDeleteGroupConfirmation.DELETE );
	}
